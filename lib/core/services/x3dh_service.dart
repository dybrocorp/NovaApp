...
class X3DHService {
  ...
  Future<X3DHResult> performX3DHAsSender({
    ...
    final signatureBytes = base64Decode(recipientBundle['signed_pre_key_signature'] ?? '');
    if (signatureBytes.isNotEmpty) {
      try {
        final valid = await _ed25519.verify(
          spkBBytes,
          signature: Signature(signatureBytes, publicKey: ikB as SimplePublicKey),
        );
        if (!valid) {
          LoggerService.warning('SPK signature verification failed', tag: 'X3DH');
        }
      } catch (e) {
    ...
    // DH1 = DH(IK_A, SPK_B)
    final dh1 = await _x25519.sharedSecretKey(keyPair: myIdentityKey, remotePublicKey: spkB);
    // DH2 = DH(EK_A, IK_B)
    ...
    if (opkB64 != null && opkB64.isNotEmpty) {
      ...
      consumedOpkId = opkB64; // The caller should track which OPK index was used
    }

    // SK = HKDF(combined)
    final sk = await _hkdf.deriveKey(secretKey: SecretKey(combined));

    LoggerService.info('X3DH completed (sender), shared secret derived', tag: 'X3DH');
  ...
  Future<List<int>> performX3DHAsReceiver({
    ...
    final ikA = SimplePublicKey(base64Decode(senderIdentityKeyBase64), type: KeyPairType.x25519);
    final ekA = SimplePublicKey(base64Decode(ephemeralPublicKeyBase64), type: KeyPairType.x25519);

    // DH1 = DH(SPK_B, IK_A)
    final dh1 = await _x25519.sharedSecretKey(keyPair: mySignedPreKey, remotePublicKey: ikA);
    // DH2 = DH(IK_B, EK_A)
    final dh2 = await _x25519.sharedSecretKey(keyPair: myIdentityKey, remotePublicKey: ekA);
    // DH3 = DH(SPK_B, EK_A)
    ...
    if (myOneTimePreKey != null) {
      final dh4 = await _x25519.sharedSecretKey(keyPair: myOneTimePreKey, remotePublicKey: ekA);
      final dh4Bytes = await dh4.extractBytes();
      combined = Uint8List.fromList([...combined, ...dh4Bytes]);
    }
    ...
    LoggerService.info('X3DH completed (receiver), shared secret derived', tag: 'X3DH');
